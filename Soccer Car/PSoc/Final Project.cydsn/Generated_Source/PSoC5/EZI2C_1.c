...
volatile uint8 EZI2C_1_curStatus; /* Current status of the component */
volatile uint8 EZI2C_1_curState;  /* Current state of state machine */

/* Primary slave address variables */
volatile uint8* EZI2C_1_dataPtrS1; /* Pointer to data buffer 1 */
volatile uint16 EZI2C_1_bufSizeS1; /* Size of buffer 1 in bytes */
volatile uint16 EZI2C_1_wrProtectS1; /* Start index of write protected area in buffer 1 */
volatile uint8  EZI2C_1_rwOffsetS1; /* Current offset within buffer 1 */
volatile uint8  EZI2C_1_rwIndexS1;  /* Current index within buffer 1 */

/* Secondary slave address variables */
#if(EZI2C_1_ADDRESSES == EZI2C_1_TWO_ADDRESSES)
    volatile uint8  EZI2C_1_addrS1; /* Primary slave address: software comparison */
    volatile uint8  EZI2C_1_addrS2; /* Secondary slave address: software comparison */
    volatile uint8* EZI2C_1_dataPtrS2; /* Pointer to data buffer 1 */
    volatile uint16 EZI2C_1_bufSizeS2; /* Size of buffer 2 in bytes */
    volatile uint16 EZI2C_1_wrProtectS2; /* Start index of write protected area in buffer 2 */
    volatile uint8 EZI2C_1_rwOffsetS2; /* Current offset within buffer 2 */
    volatile uint8 EZI2C_1_rwIndexS2;  /* Current index within buffer 2 */
...
*
* Global variables:
*  EZI2C_1_initVar - It is used to indicate the initial configuration
*  of this component.  The variable is initialized to zero and set to 1
*  the first time EZI2C_1_Start() is called. This allows component  !
*  initialization without re-initialization in all subsequent calls
*  to the EZI2C_1_Start() routine.
*
*  EZI2C_1_curStatus - Stores the current status of the component.
*
*  EZI2C_1_curState- Stores the current state of an I2C state machine.
*
*  EZI2C_1_rwOffsetS1 - The global variable which stores an offset for read
...
*
*  EZI2C_1_rwIndexS1 - The global variable which stores a pointer to the
...
*
* EZI2C_1_wrProtectS1 - The global variable which stores an offset where data
*  is read only for the first slave address is reset if
*  EZI2C_1_initVar is 0, by EZI2C_1_initVar function call.
*
*  EZI2C_1_rwOffsetS2 - The global variable, which stores an offset for read
...
*  function call.
*
*  EZI2C_1_rwIndexS2 - The global variable which stores a pointer to the
*  next value to be read or written for the second slave address is reset if
*  EZI2C_1_initVar is 0, by EZI2C_1_initVar function call.
*
* EZI2C_1_wrProtectS2 - The global variable which stores an offset where data
...
void EZI2C_1_Init(void) 
{
    ...
    EZI2C_1_CFG_REG  = EZI2C_1_DEFAULT_CFG;
    EZI2C_1_XCFG_REG = EZI2C_1_DEFAULT_XCFG;
    EZI2C_1_ADDR_REG = EZI2C_1_DEFAULT_ADDR;
    EZI2C_1_CLKDIV1_REG = EZI2C_1_DEFAULT_CLKDIV1;
    EZI2C_1_CLKDIV2_REG = EZI2C_1_DEFAULT_CLKDIV2;

#if (EZI2C_1_TWO_ADDRESSES == EZI2C_1_ADDRESSES)
    /* Set address variables */
    EZI2C_1_SetAddress1(EZI2C_1_DEFAULT_ADDR1);
    EZI2C_1_SetAddress2(EZI2C_1_DEFAULT_ADDR2);
#endif /* #if (EZI2C_1_TWO_ADDRESSES == EZI2C_1_ADDRESSES) */
...
void EZI2C_1_Stop(void) 
{
    ...
    blockResetCycles = ((uint16) ((uint16) EZI2C_1_CLKDIV2_REG << 8u) | EZI2C_1_CLKDIV1_REG) + 1u;
...
*
* Global variables:
*  EZI2C_1_addrS1 - A new slave address for the first slave device is
...
*
*   EZI2C_1_STATUS_READ1   Set if a Read sequence is detected for the first
*                                   address. Cleared when the status is read.
*
*   EZI2C_1_STATUS_WRITE1  Set if a Write sequence is detected for the first
*                                   address. Cleared when the status is  read.
*
*   EZI2C_1_STATUS_READ2   Set if a Read sequence is detected for the second
*                                   address (if enabled). Cleared when the status
*                                   read.
*
*   EZI2C_1_STATUS_WRITE2  Set if a Write sequence is detected for the second
*                                   address (if enabled). Cleared when the status is
*                                   read.
*
*   EZI2C_1_STATUS_BUSY    Set when the master starts communication with
...
*
*   EZI2C_1_STATUS_ERR     Set when the I2C hardware is detected, cleared
*                                   when the status is read.
*
* Global variables:
*  EZI2C_1_curStatus - The global variable which stores the current
*  component status.
*
* Reentrant:
*  No.
...
uint8 EZI2C_1_GetActivity(void) 
{
    ...
    EZI2C_1_DisableInt(); /* Lock from component interrupt */
...
*
* Global variables:
*  EZI2C_1_dataPtrS1 - Stores the pointer to the data exposed to an I2C
...
*
*  EZI2C_1_rwOffsetS1 - Stores an offset for read and write operations, is
*  modified at each write sequence of the first slave address is?? reset.   ?
*
*  EZI2C_1_rwIndexS1 - Stores the pointer to the next value to be read or
...
void EZI2C_1_SetBuffer1(uint16 bufSize, uint16 rwBoundary, volatile uint8 * dataPtr) 
{
    ...
    {
        /* Configure buffer 1 */
        EZI2C_1_dataPtrS1   = dataPtr;
        EZI2C_1_bufSizeS1   = bufSize;
        EZI2C_1_wrProtectS1 = rwBoundary;
    }
    ...
    *
    * Global variables:
    *  EZI2C_1_dataPtrS2 - Stores the pointer to the data exposed to an I2C
    *  master for the second slave address, is modified with the the new pointer to the
    *  data, passed by the unction parameter.
    *
    *  EZI2C_1_rwOffsetS2 - Stores teh offset for read and write operations,
    *  is modified at each write sequence of the second slave address is?? reset.  ??
    *
    *  EZI2C_1_rwIndexS2 - Stores the pointer to the next value to be read or
    ...
    *******************************************************************************/
    void EZI2C_1_SetBuffer2(uint16 bufSize, uint16 rwBoundary, volatile uint8 * dataPtr) 
    {
        ...
        {
            /* Configure buffer 2 */
            EZI2C_1_dataPtrS2   = dataPtr;
            EZI2C_1_bufSizeS2   = bufSize;
            EZI2C_1_wrProtectS2 = rwBoundary;
        }
    ...
    *
    * Parameters:
    *  EZI2C_1_addrS2 - The global variable which stores the second I2C
    *   address.
    *
    * Return:
