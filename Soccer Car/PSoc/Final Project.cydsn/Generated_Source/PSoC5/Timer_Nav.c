...
void Timer_Nav_Init(void) 
{
        ...
        #if (CY_PSOC5A)
            Timer_Nav_CONTROL2 &= ((uint8)(~Timer_Nav_CTRL_MODE_MASK));
        #endif /* Clear bits in CONTROL2 only in PSOC5A */

        #if (CY_PSOC3 || CY_PSOC5LP)
            Timer_Nav_CONTROL3 &= ((uint8)(~Timer_Nav_CTRL_MODE_MASK));
        ...
        /* Check if One Shot mode is enabled i.e. RunMode !=0*/
            ...
            Timer_Nav_CONTROL |= 0x04u;
                ...
                Timer_Nav_CONTROL3 &= ((uint8)(~Timer_Nav_CTRL_RCOD_MASK));
                Timer_Nav_CONTROL3 |= Timer_Nav_CTRL_RCOD;

                /* Clear and Enable the HW enable bit in CFG2 register */
                Timer_Nav_CONTROL3 &= ((uint8)(~Timer_Nav_CTRL_ENBL_MASK));
        ...
        Timer_Nav_RT1 &= ((uint8)(~Timer_Nav_RT1_MASK));
        Timer_Nav_RT1 |= Timer_Nav_SYNC;

        /*Enable DSI Sync all all inputs of the Timer*/
        Timer_Nav_RT1 &= ((uint8)(~Timer_Nav_SYNCDSI_MASK));
    ...
    Timer_Nav_WriteCounter(Timer_Nav_INIT_PERIOD);
        ...
        Timer_Nav_SetCaptureCount(Timer_Nav_INIT_CAPTURE_COUNT);
            ...
            Timer_Nav_SetCaptureMode(Timer_Nav_INIT_CAPTURE_MODE);
                ...
                if (0u == (Timer_Nav_CONTROL & Timer_Nav__B_TIMER__TM_SOFTWARE))
                {
        ...
        #if (Timer_Nav_InterruptOnCaptureCount && !Timer_Nav_UDB_CONTROL_REG_REMOVED)
...
void Timer_Nav_Enable(void) 
{
    ...
    #if (!Timer_Nav_UDB_CONTROL_REG_REMOVED || Timer_Nav_UsingFixedFunction)
        Timer_Nav_CONTROL |= Timer_Nav_CTRL_ENABLE;
...
void Timer_Nav_Stop(void) 
{
    /* Disable Timer */
    #if(!Timer_Nav_UDB_CONTROL_REG_REMOVED || Timer_Nav_UsingFixedFunction)
        Timer_Nav_CONTROL &= ((uint8)(~Timer_Nav_CTRL_ENABLE));
    #endif /* Remove assignment if control register is removed */

    /* Globally disable the Fixed Function Block chosen */
    #if (Timer_Nav_UsingFixedFunction)
        Timer_Nav_GLOBAL_ENABLE &= ((uint8)(~Timer_Nav_BLOCK_EN_MASK));
        Timer_Nav_GLOBAL_STBY_ENABLE &= ((uint8)(~Timer_Nav_BLOCK_STBY_EN_MASK));
...
void Timer_Nav_SoftwareCapture(void) 
{
    ...
    #if(Timer_Nav_UsingFixedFunction)
        (void)CY_GET_REG16(Timer_Nav_COUNTER_LSB_PTR);
    #else
        (void)CY_GET_REG8(Timer_Nav_COUNTER_LSB_PTR_8BIT);
...
#if (!Timer_Nav_UDB_CONTROL_REG_REMOVED) /* Remove API if control register is unused */
...
{
    #if (!Timer_Nav_UDB_CONTROL_REG_REMOVED) 
        return ((uint8)Timer_Nav_CONTROL);
    #else
        return (0);
    #endif /* (!Timer_Nav_UDB_CONTROL_REG_REMOVED) */
}
...
{
   #if(Timer_Nav_UsingFixedFunction)
       return ((uint16)CY_GET_REG16(Timer_Nav_PERIOD_LSB_PTR));
   #else
...
{
   #if(Timer_Nav_UsingFixedFunction)
       return ((uint16)CY_GET_REG16(Timer_Nav_CAPTURE_LSB_PTR));
   #else
...
void Timer_Nav_WriteCounter(uint16 counter) 
{
         ...
         */
        CY_SET_REG16(Timer_Nav_COUNTER_LSB_PTR, (uint16)counter);
...
void Timer_Nav_SetCaptureMode(uint8 captureMode) 
{
    ...
    captureMode = ((uint8)((uint8)captureMode << Timer_Nav_CTRL_CAP_MODE_SHIFT));
    captureMode &= (Timer_Nav_CTRL_CAP_MODE_MASK);

    #if (!Timer_Nav_UDB_CONTROL_REG_REMOVED)
        /* Clear the Current Setting */
        Timer_Nav_CONTROL &= ((uint8)(~Timer_Nav_CTRL_CAP_MODE_MASK));
...
void Timer_Nav_SetTriggerMode(uint8 triggerMode) 
{
    ...
    #if (!Timer_Nav_UDB_CONTROL_REG_REMOVED)   /* Remove assignment if control register is removed */
    
        /* Clear the Current Setting */
        Timer_Nav_CONTROL &= ((uint8)(~Timer_Nav_CTRL_TRIG_MODE_MASK));
...
{
    #if (!Timer_Nav_UDB_CONTROL_REG_REMOVED )   /* Remove assignment if control register is removed */
        Timer_Nav_CONTROL &= ((uint8)(~Timer_Nav_CTRL_TRIG_EN));
    #endif /* Remove code section if control register is not used */
}
...
void Timer_Nav_SetInterruptCount(uint8 interruptCount) 
{
    ...
    #if (!Timer_Nav_UDB_CONTROL_REG_REMOVED)
        /* Clear the Current Setting */
        Timer_Nav_CONTROL &= ((uint8)(~Timer_Nav_CTRL_INTCNT_MASK));
        /* Write The New Setting */
...
{
    return ((uint8)Timer_Nav_CAP_COUNT);
}
...
{
    while(0u != (Timer_Nav_ReadStatusRegister() & Timer_Nav_STATUS_FIFONEMP))
    {
        (void)Timer_Nav_ReadCapture();
    }
}
